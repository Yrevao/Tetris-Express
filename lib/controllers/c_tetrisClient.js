...
module.exports = {
    ...
    updateState: (req, res, io) => {
        ...
        const matchId = s_player.findByKey(playerId).match;
        s_player.findByKeyAndUpdate(playerId, 'board', req.body.board);

        //io.to(matchId).emit('update', { players: s_player.findManyByProperty('match', matchId) });

        res.json({status: 'ok'});
    },
    joinSocket: (socket, io) => {
        s_player.findByKeyAndUpdate(socket.id, 'socket', socket);
        const matchId = s_player.findByKey(socket.id).match;
        if(!matchId)
            return;
